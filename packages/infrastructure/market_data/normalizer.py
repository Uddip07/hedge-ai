...
class TickerNormalizer:
    ...
    # Exchange suffix mappings for standard Yahoo Finance vendor feeds
    ...
    def to_provider_symbol(
        ...
        Args:
            ticker (Ticker): Domain Ticker value object (e.g. RELIANCE.NSE or INFY.NSE).
            provider (str): Targeted provider key ("yahoo", "nse", "bse", etc.).

        Returns:
            str: Vendor normalized ticker string (e.g., "RELIANCE.NS").
        """
        ...
        if provider.lower() in ("yfinance", "yahoo"):
            ...
            # Avoid double suffix if symbol already includes it
            if suffix and not base_symbol.endswith(suffix):
                return f"{base_symbol}{suffix}"
            return base_symbol

        if provider.lower() == "nse":
            return base_symbol

        if provider.lower() == "bse":
            return base_symbol

        # Default fallback: return full symbol or symbol base
        return base_symbol
    ...
    def from_provider_symbol(
        ...
        Args:
            symbol (str): Vendor symbol string (e.g., "RELIANCE.NS", "AAPL").
            default_exchange (ExchangeType): Exchange fallback if unspecified.

        Returns:
            Ticker: Domain Ticker object.
        """
