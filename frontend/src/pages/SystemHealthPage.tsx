...
export const SystemHealthPage: React.FC = () => {
  const { health, root, version } = useSystemHealth();

  if (health.isLoading || root.isLoading || version.isLoading) {
    return <LoadingSpinner message="Auditing connected providers and subsystem health..." />;
  }

  if (health.isError || root.isError || version.isError) {
    return (
      <ErrorAlert
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 font-mono text-xs">
        <div className="rounded-2xl border border-slate-800/80 bg-slate-900/90 p-5 backdrop-blur-md shadow-xl">
