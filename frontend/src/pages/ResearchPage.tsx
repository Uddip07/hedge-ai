...
import { SearchCode, Send, FileText, Bookmark, Bot, User } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 w-full max-w-full min-w-0 font-mono">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#121826] p-5 rounded-2xl border border-[#1E293B]">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Contextual document retrieval across SEC filings, earnings transcripts, and equity research reports.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Chat Stream Column */}
        <div className="lg:col-span-2 bg-[#121826] rounded-2xl p-5 border border-[#1E293B] flex flex-col h-[560px] min-w-0">
          {/* Messages Container */}
          <div className="flex-1 overflow-y-auto space-y-4 pr-2">
            {messages.map((m) => (
              <div key={m.id} className={`flex gap-3 ${m.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                {m.sender === 'ai' && (
                  <div className="p-2 rounded-xl bg-cyan-950/60 border border-cyan-800/60 text-cyan-400 h-fit shrink-0">
                ...
                <div
                  className={`max-w-xl p-4 rounded-2xl text-xs space-y-2 leading-relaxed ${
                    m.sender === 'user'
                      ? 'bg-cyan-950/80 text-cyan-100 border border-cyan-800/60 rounded-tr-none'
                      : 'bg-slate-900 text-slate-200 border border-[#1E293B] rounded-tl-none'
                  }`}
                >
                  <p>{m.text}</p>
                  {m.citations && (
                    <div className="pt-2 border-t border-[#1E293B] flex flex-wrap gap-1.5 text-[10px]">
                      <span className="text-slate-400">Sources:</span>
                      {m.citations.map((c, i) => (
                        <span
                          key={i}
                          className="px-2 py-0.5 rounded bg-slate-800 text-cyan-300 border border-slate-700 flex items-center gap-1"
                        >
                ...
                {m.sender === 'user' && (
                  <div className="p-2 rounded-xl bg-slate-800 border border-slate-700 text-slate-300 h-fit shrink-0">
          ...
          {/* Input Form */}
          <form onSubmit={handleSend} className="mt-4 pt-3 border-t border-[#1E293B] flex items-center gap-2">
            <input
              type="text"
              placeholder="Ask RAG assistant about balance sheets, ROCE, or SEC transcripts..."
              value={inputQuery}
              onChange={(e) => setInputQuery(e.target.value)}
              className="flex-1 bg-slate-900 border border-[#1E293B] rounded-xl px-4 py-3 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500/50"
            />
            <button
              type="submit"
              className="p-3 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold transition-all shadow-lg shadow-cyan-950/50 shrink-0"
            >
        ...
        <div className="bg-[#121826] rounded-2xl p-5 border border-[#1E293B] space-y-4 min-w-0">
          ...
          <div className="space-y-3 text-xs">
            {[
              ...
              <div key={i} className="p-3 rounded-xl bg-slate-900 border border-[#1E293B] space-y-1">
